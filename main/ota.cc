...
std::string Ota::GetCheckVersionUrl() {
    ...
    std::string url = settings.GetString("ota_url_default");
    if (url.empty()) {
        url = CONFIG_OTA_URL;
    }
    return url;
...
bool Ota::CheckVersion(std::string& url) {
    ...
    if (cJSON_IsObject(server_time)) {
        ...
        if (cJSON_IsNumber(timestamp)) {
            // server_time.timestamp is unix epoch in milliseconds (UTC).
            // Do NOT add timezone offset here; local timezone conversion
            // must be handled by TZ/localtime only.
            struct timeval tv;
