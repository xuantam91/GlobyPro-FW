...
namespace {
std::string NormalizeSpeechCommand(const std::string& input) {
    std::string out;
...
bool CustomWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    std::deque<Command> registered_commands;
    std::unordered_set<std::string> seen_commands;
    for (const auto& command : commands_) {
        std::vector<std::string> candidates = {
            NormalizeSpeechCommand(command.command),
            NormalizeSpeechCommand(command.text),
        };

        bool registered_any = false;
        for (const auto& candidate : candidates) {
            if (candidate.empty() || seen_commands.count(candidate) > 0) {
                continue;
            }
            esp_err_t err = esp_mn_commands_add(
                static_cast<int>(registered_commands.size()) + 1,
                candidate.c_str());
            if (err == ESP_OK) {
                registered_commands.push_back({candidate, command.text, command.action});
                seen_commands.insert(candidate);
                registered_any = true;
                ESP_LOGI(TAG, "Registered wake command: %s", candidate.c_str());
            } else {
                ESP_LOGW(TAG, "Rejected wake command: %s", candidate.c_str());
            }
        }

        if (!registered_any) {
            ESP_LOGE(TAG, "No valid command variant for wake text: %s", command.text.c_str());
        }
    }

    if (registered_commands.empty()) {
        ESP_LOGE(TAG, "No valid custom wake commands available");
        return false;
    }
    commands_.swap(registered_commands);
