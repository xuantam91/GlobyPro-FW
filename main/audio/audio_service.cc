...
int64_t GetOutputPowerTimeoutMs() {
    return IsLuxiaoban154Board() ? 60LL * 60LL * 1000LL : AUDIO_POWER_TIMEOUT_MS;
}
}  // namespace
...
void AudioService::CheckAndUpdateAudioPowerState() {
    ...
    // Keep MIC input always on so wake word can always be detected in idle mode.
    if (!codec_->input_enabled()) {
        ESP_LOGW(TAG, "%s Re-enabling audio input to keep wake word active", __func__);
        codec_->EnableInput(true);
    }
    if (output_elapsed > GetOutputPowerTimeoutMs() && codec_->output_enabled()) {
