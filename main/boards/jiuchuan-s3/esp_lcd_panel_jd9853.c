...
static esp_err_t panel_jd9853_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(jd9853->io, command, NULL, 0), TAG, "Invert failed");
    return ESP_OK;
}
...
static esp_err_t panel_jd9853_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(jd9853->io, LCD_CMD_MADCTL, (uint8_t[]) {
        jd9853->madctl_val
    }, 1), TAG, "MADCTL failed");
