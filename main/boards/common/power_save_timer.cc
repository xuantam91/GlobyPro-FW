...
bool IsPowerSaveEnabledFromSettings() {
...
int GetPowerOffTimeoutSecondsFromSettings() {
    Settings power_settings("power", false);
...
void PowerSaveTimer::PowerSaveCheck() {
    if (!IsPowerSaveEnabledFromSettings()) {
        ticks_ = 0;
        if (in_sleep_mode_) {
            WakeUp();
        }
    ...
    const int runtime_shutdown_seconds = GetPowerOffTimeoutSecondsFromSettings();
    ...
    if (seconds_to_sleep_ != -1 && ticks_ >= seconds_to_sleep_) {
        if (!in_sleep_mode_) {
            ...
            if (cpu_max_freq_ != -1) {
                // Keep wake word active in sleep mode so voice wake-up still works.
                auto& audio_service = app.GetAudioService();
                is_wake_word_running_ = audio_service.IsWakeWordRunning();
                if (is_wake_word_running_) {
                    ESP_LOGI(TAG, "Keeping wake word detection active in power save mode");
                } else {
                    // No wake word is active, so input can be disabled to save power.
    ...
    const int effective_shutdown_seconds =
        (runtime_shutdown_seconds > 0) ? runtime_shutdown_seconds : seconds_to_shutdown_;
    if (effective_shutdown_seconds != -1 && ticks_ >= effective_shutdown_seconds && on_shutdown_request_) {
